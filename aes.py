...
mul3 = [[0x00, 0x03, 0x06, 0x05, 0x0c, 0x0f, 0x0a, 0x09, 0x18, 0x1b,
            0x1e, 0x1d, 0x14, 0x17, 0x12, 0x11],
...
def key_expansion(key, numround, rci, s_box):
...
def mix_col(mul2, mul3, pt):
        ...
        for i in range(4):
            ...
            temp.append(pt[i][2])
            temp.append(pt[i][3])
            res.append(mix_mul_col(temp, mul2, mul3))
        return res
...
print_matrix(initialState)
print("Initial Key matrix")
print_matrix(initialKey)
initialState = add_round_key(initialState, initialKey)
print_matrix(initialState)
for i in range(1, 11):
    initialKey = key_expansion(initialKey, i, rci, s_box)
    initialState = substitute(initialState, s_box)
    initialState = shift_row(initialState)
    if(i != 10):
        initialState = mix_col(mul2, mul3, initialState)
    initialState = add_round_key(initialState, initialKey)
print("-----------Result----------")
print("Original Message")
print_cipher(finalState)
print("The cipher text is")
print_cipher(initialState)
temp = ""
for i in range(4):
    for j in range(4):
        y = initialState[i][j]
